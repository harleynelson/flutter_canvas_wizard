// File: lib/widgets/editor_canvas.dart
// Description: Renders the models and overlays interactive editing elements, now with Ghosting support for evaluated timeline logic.
...
import 'interactive_canvas.dart'; // Import HandleType for Hover State matching
...
class EditorCanvasPainter extends CustomPainter {
  ...
  // NEW: Added to support clean Image Exports
  ...
  EditorCanvasPainter({
    ...
    this.isExportMode = false, // defaults to false
  });
  ...
  void paint(Canvas canvas, Size size) {
    try {
      ...
      // Hide grid and axis when exporting PNG
      if (!isExportMode) {
      ...
      // Hide highlights and transform boxes when exporting PNG
      if (!isExportMode) {
        if (hoveredItemId != null && hoveredItemId != selectedItemId) {
          final hoveredItem = _findItemRecursive(items, hoveredItemId);
          if (hoveredItem != null) _drawHoverHighlight(canvas, hoveredItem);
        }

        final selectedItem = _findItemRecursive(items, selectedItemId);
        if (selectedItem != null) {
          if (isTransformMode) {
             _drawTransformBox(canvas, selectedItem);
          } else {
             _drawSelectionHighlight(canvas, selectedItem);
          }
        }
      }
  ...
  void _renderItem(Canvas canvas, CanvasItem item, {bool inheritedGhost = false}) {
    ...
    // Evaluate logic for "Ghosting"
    bool isGhost = inheritedGhost || !ExpressionEvaluator.evaluate(item.enabledIf, variables);

    try {
      if (item is RectItem) {
        _drawRectItem(canvas, item, isGhost);
      } else if (item is RRectItem) {
        _drawRRectItem(canvas, item, isGhost);
      } else if (item is OvalItem) {
        _drawOvalItem(canvas, item, isGhost);
      } else if (item is PathItem) {
        _drawPathItem(canvas, item, isGhost);
      } else if (item is TextItem) {
        _drawTextItem(canvas, item, isGhost);
      } else if (item is LogicGroupItem) {
        for (var child in item.children) {
          _renderItem(canvas, child, inheritedGhost: isGhost);
        }
      }
    } catch (e) {
  ...
  void _drawRRectItem(Canvas canvas, RRectItem item, bool isGhost) {
    try {
      ...
      for (int i = item.paint.extrusionSteps; i >= 0; i--) {
        ...
        if (sWidth > 0 || isGhost) {
          canvas.drawRRect(rrect, paint..color = strokeColor..style = PaintingStyle.stroke..strokeWidth = sWidth);
        }
      }
    } catch (e) {
  ...
  void _drawOvalItem(Canvas canvas, OvalItem item, bool isGhost) {
    try {
      ...
      for (int i = item.paint.extrusionSteps; i >= 0; i--) {
        ...
        if (item.paint.fillColor != Colors.transparent) {
          canvas.drawOval(rect, paint..color = i == 0 ? fillColor : fillColor.withOpacity(isGhost ? 0.05 : 0.8)..style = PaintingStyle.fill);
        }

        if (sWidth > 0 || isGhost) {
          canvas.drawOval(rect, paint..color = strokeColor..style = PaintingStyle.stroke..strokeWidth = sWidth);
        }
      }
    } catch (e) {
  ...
  void _drawTransformBox(Canvas canvas, CanvasItem selectedItem) {
    try {
       final Rect bounds = BoundingBoxUtils.getCombinedRect([selectedItem]);
       if (bounds == Rect.zero) return;

       // Inflate slightly to wrap outside the shape
  ...
  void _drawRectItem(Canvas canvas, RectItem item, bool isGhost) {
    try {
      ...
      for (int i = item.paint.extrusionSteps; i >= 0; i--) {
        final offset = Offset(
          item.paint.extrusionOffset.dx * i,
          item.paint.extrusionOffset.dy * i,
        );
        final rect = item.rect.shift(offset);
        ...
        if (item.paint.fillColor != Colors.transparent) {
          canvas.drawRect(
            rect,
            paint..color = i == 0 ? fillColor : fillColor.withOpacity(isGhost ? 0.05 : 0.8)
  ...
  void _drawHoverHighlight(Canvas canvas, CanvasItem item) {
    try {
      final hoverPaint = Paint()
        ...
        ..style = PaintingStyle.stroke
        ..strokeWidth = 4 / cameraZoom;
        
      if (item is RectItem) {
        canvas.drawRect(item.rect, hoverPaint);
      } else if (item is RRectItem) {
        canvas.drawRRect(RRect.fromRectAndRadius(item.rect, Radius.circular(item.radius)), hoverPaint);
      } else if (item is OvalItem) {
        canvas.drawOval(item.rect, hoverPaint);
      } else if (item is PathItem) {
        final path = BezierPathData(nodes: item.nodes, isClosed: item.isClosed).generatePath();
      ...
      if (selectedItem is RectItem || selectedItem is RRectItem || selectedItem is OvalItem) {
        ...
        try {
          ...
          if (hoveredItemId == selectedItem.id && hoveredHandle == HandleType.pathEdge && hoveredNodeIndex != null) {
            ...
            final current = selectedItem.nodes[i];
            final next = selectedItem.nodes[nextIndex];
            
            final segmentPath = Path()..moveTo(current.position.dx, current.position.dy);
            if (current.controlPoint2 != null && next.controlPoint1 != null) {
              segmentPath.cubicTo(current.controlPoint2!.dx, current.controlPoint2!.dy, next.controlPoint1!.dx, next.controlPoint1!.dy, next.position.dx, next.position.dy);
            } else if (current.controlPoint2 != null || next.controlPoint1 != null) {
              final cp = current.controlPoint2 ?? next.controlPoint1!;
              segmentPath.quadraticBezierTo(cp.dx, cp.dy, next.position.dx, next.position.dy);
            } else {
              segmentPath.lineTo(next.position.dx, next.position.dy);
            }
            canvas.drawPath(segmentPath, Paint()..color = Colors.yellowAccent.withOpacity(0.8)..style = PaintingStyle.stroke..strokeWidth = 4.0 / cameraZoom);
          }
        } catch (e) {
          print('DEBUG ERROR: Path skeleton highlight failed: $e');
        }
        ...
        if (hoveredItemId == selectedItem.id && hoveredHandle == HandleType.pathEdge && hoverPos != null) {
          final addBg = Paint()..color = Colors.green..style = PaintingStyle.fill;
          final addFg = Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 2 / cameraZoom;
          final btnRadius = 8 / cameraZoom;
          final lineLen = 4 / cameraZoom;

          canvas.drawCircle(hoverPos!, btnRadius, addBg);
          canvas.drawLine(hoverPos! + Offset(-lineLen, 0), hoverPos! + Offset(lineLen, 0), addFg); 
          canvas.drawLine(hoverPos! + Offset(0, -lineLen), hoverPos! + Offset(0, lineLen), addFg); 
        }
      } 
      else if (selectedItem is LogicGroupItem) {
        if (selectedItem.children.isNotEmpty) {
          final rect = BoundingBoxUtils.getCombinedRect(selectedItem.children);
          
          if (rect != Rect.zero) {
            final padding = 8.0 / cameraZoom;
            final paddedRect = rect.inflate(padding);
            ...
            canvas.drawRect(
              ...
              Paint()
                ...
                ..style = PaintingStyle.stroke
                ..strokeWidth = 2.0 / cameraZoom
            );

            // CONDITIONAL TEXT HIDING LOGIC IMPLEMENTED HERE
            if (selectedItem.condition != 'true' && selectedItem.condition.trim().isNotEmpty) {
              final textPainter = TextPainter(
                text: TextSpan(
                  text: 'if (${selectedItem.condition})', 
                  style: TextStyle(
                    color: Colors.orangeAccent, 
                    fontSize: 12 / cameraZoom, 
                    fontWeight: FontWeight.bold,
